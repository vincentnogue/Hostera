  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Maintenance</h1>
          <p className="text-sm text-[#64748B] mt-1">Track and manage maintenance tickets</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-3">
          ...
          <div><p className="text-2xl font-bold text-[#17212B]">{stats.open}</p><p className="text-xs text-[#64748B]">Open</p></div>
        </div>
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center">
            <Wrench className="w-5 h-5 text-amber-600" />
          </div>
          <div><p className="text-2xl font-bold text-[#17212B]">{stats.in_progress}</p><p className="text-xs text-[#64748B]">In Progress</p></div>
        </div>
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-green-50 flex items-center justify-center">
            <CheckCircle2 className="w-5 h-5 text-green-600" />
          </div>
          <div><p className="text-2xl font-bold text-[#17212B]">{stats.completed}</p><p className="text-xs text-[#64748B]">Completed</p></div>
        </div>
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center">
            <AlertCircle className="w-5 h-5 text-red-600" />
          </div>
          <div><p className="text-2xl font-bold text-[#17212B]">{stats.urgent}</p><p className="text-xs text-[#64748B]">Urgent</p></div>
        </div>
      </div>
        ...
        {['all', 'open', 'in_progress', 'completed'].map(s => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              statusFilter === s ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:bg-[#F6F8FB]'
            }`}
          >
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            <Wrench className="w-12 h-12 text-[#E2E8F0] mx-auto mb-3" />
            <p className="text-sm text-[#64748B]">No maintenance tickets</p>
          </div>
        ) : (
          <div className="divide-y divide-[#E2E8F0]">
            {filtered.map((ticket) => {
              ...
              return (
                <div key={ticket.id} className="p-4 hover:bg-[#F6F8FB] transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-sm font-semibold text-[#17212B]">{ticket.title}</h3>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${priorityColors[ticket.priority]}`}>
                          {ticket.priority}
                        </span>
                      </div>
                      {ticket.description && (
                        <p className="text-xs text-[#64748B] mb-1">{ticket.description}</p>
                      )}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <select
                        ...
                        className="text-xs px-2 py-1 border border-[#E2E8F0] rounded-lg outline-none cursor-pointer bg-white"
                      >
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[#17212B]">New Maintenance Ticket</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-[#17212B] mb-1 block">Title</label>
                <input
                  type="text"
                  value={form.title}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-[#17212B] mb-1 block">Priority</label>
                  <select
                    value={form.priority}
                ...
                <div>
                  <label className="text-sm font-medium text-[#17212B] mb-1 block">Category</label>
                  <select
                    value={form.category}
              ...
              <div>
                <label className="text-sm font-medium text-[#17212B] mb-1 block">Room (optional)</label>
                <select
                  value={form.room_id}
