  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand-ink">Document Center</h1>
          <p className="text-sm text-brand-slate">Registration forms, welcome letters and invoice templates — organized for automated guest distribution.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setType('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${type === 'all' ? 'bg-brand-navy text-white' : 'bg-white border border-brand-border text-brand-slate hover:border-brand-navy'}`}>All</button>
        {types.map(t => (
          <button key={t} onClick={() => setType(t)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${type === t ? 'bg-brand-navy text-white' : 'bg-white border border-brand-border text-brand-slate hover:border-brand-navy'}`}>
      ...
      {loading ? (
        <p className="text-sm text-brand-slate">Loading templates…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-brand-border p-10 text-center">
          ...
          <p className="text-sm text-brand-slate">No templates yet — create your first document template.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(d => (
            <div key={d.id} className="bg-white rounded-xl border border-brand-border p-5 flex flex-col">
              ...
              <p className="text-sm font-semibold text-brand-ink">{d.name}</p>
              <div className="flex gap-1.5 mt-2 mb-3">
                <span className="text-[10px] px-2 py-0.5 bg-blue-50 text-brand-navy rounded-full font-medium">{typeLabels[d.type] || d.type}</span>
                <span className="text-[10px] px-2 py-0.5 bg-brand-bg text-brand-slate rounded-full font-medium uppercase flex items-center gap-1"><Globe className="w-3 h-3" />{d.language || 'en'}</span>
              </div>
              <p className="text-[12px] text-brand-slate leading-relaxed line-clamp-3 flex-1">{d.content || 'No content yet.'}</p>
              <div className="flex items-center justify-between pt-3 border-t border-[#F1F5F9] mt-3">
                <button onClick={() => openEdit(d)} className="text-[11px] px-3 py-1.5 border border-brand-border text-brand-slate rounded-full hover:border-brand-navy hover:text-brand-navy font-medium">Edit</button>
                <div className="flex gap-1.5">
                  {d.status !== 'active' && <button onClick={() => setStatus(d, 'active')} className="flex items-center gap-1 text-[11px] px-3 py-1.5 bg-brand-navy text-white rounded-full font-semibold hover:bg-brand-blue"><Send className="w-3 h-3" /> Activate</button>}
                  {d.status !== 'archived' && <button onClick={() => setStatus(d, 'archived')} className="flex items-center gap-1 text-[11px] px-3 py-1.5 border border-brand-border text-brand-slate rounded-full hover:border-red-200 hover:text-red-600"><Archive className="w-3 h-3" /> Archive</button>}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-brand-ink">{editing === 'new' ? 'New Template' : 'Edit Template'}</h3>
              <button onClick={() => setEditing(null)}><X className="w-4 h-4 text-brand-slate" /></button>
            </div>
            <form onSubmit={save} className="space-y-3">
              ...
              <textarea placeholder="Template content — use variables like {{guest_name}}, {{room_number}}, {{check_in}}…" value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={7} className="w-full px-4 py-3 border border-brand-border rounded-3xl text-sm outline-none focus:border-brand-navy resize-none" />
              <button type="submit" className="w-full py-2.5 bg-brand-navy text-white text-sm font-semibold rounded-full hover:bg-brand-blue">Save Template</button>
            </form>
          </div>
        </div>
      )}
