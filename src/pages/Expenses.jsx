  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Expense Tracking</h1>
          <p className="text-sm text-[#64748B]">Property operational costs, linked to vendors, with approval workflow.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'This Month', value: `$${monthTotal.toLocaleString()}`, icon: Banknote, tint: 'text-[#123B63]' },
          { label: 'Pending Approval', value: pending.length, icon: CalendarDays, tint: 'text-amber-500' },
          { label: 'Top Category', value: topCat ? topCat[0].replace('_', ' ') : '—', icon: TrendingDown, tint: 'text-[#1F5A8A]' },
          ...
          return (
            <div key={k.label} className="bg-white rounded-xl border border-[#E2E8F0] p-4">
              ...
              <p className="text-lg font-bold text-[#17212B] capitalize">{k.value}</p>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setCat('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${cat === 'all' ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>All Categories</button>
        {categories.map(c => (
          <button key={c} onClick={() => setCat(c)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${cat === c ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>{c.replace('_', ' ')}</button>
        ))}
        {vendors.length > 0 && (
          <>
            <span className="w-px h-6 bg-[#E2E8F0] mx-1 self-center" />
            <button onClick={() => setVendor('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${vendor === 'all' ? 'bg-[#1F5A8A] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>All Vendors</button>
            {vendors.map(v => (
              <button key={v} onClick={() => setVendor(v)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${vendor === v ? 'bg-[#1F5A8A] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>{v}</button>
            ))}
          </>
        )}
      </div>
      ...
      {loading ? (
        <p className="text-sm text-[#64748B]">Loading expenses…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-[#E2E8F0] p-10 text-center">
          ...
          <p className="text-sm text-[#64748B]">No expenses recorded yet.</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map(e => (
                <tr key={e.id} className="border-b border-[#F1F5F9] last:border-0">
                  <td className="px-5 py-3.5 text-[#64748B] whitespace-nowrap">{e.expense_date ? new Date(e.expense_date).toLocaleDateString() : '—'}</td>
                  <td className="px-5 py-3.5 font-medium text-[#17212B]">{e.description}</td>
                  <td className="px-5 py-3.5"><span className="text-[11px] px-2.5 py-1 rounded-full bg-[#F6F8FB] text-[#64748B] border border-[#E2E8F0] capitalize">{e.category?.replace('_', ' ')}</span></td>
                  <td className="px-5 py-3.5 text-[#64748B]">{e.vendor || '—'}</td>
                  <td className="px-5 py-3.5 font-semibold text-[#17212B]">${(e.amount || 0).toLocaleString()}</td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#17212B]">Record Expense</h3>
            ...
            <form onSubmit={addExpense} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Record Expense</button>
            </form>
          </div>
        </div>
      )}
