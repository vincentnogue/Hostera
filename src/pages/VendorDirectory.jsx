...
const catPills = {
  laundry: 'bg-blue-50 text-brand-navy', food_beverage: 'bg-orange-50 text-orange-700', repair: 'bg-amber-50 text-amber-700',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand-ink">Vendor Directory</h1>
          <p className="text-sm text-brand-slate">External suppliers — laundry, food, repair services — with contacts and performance notes.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={k.label} className="bg-white rounded-xl border border-brand-border p-4">
            <p className="text-lg font-bold text-brand-ink truncate">{k.value}</p>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setCat('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${cat === 'all' ? 'bg-brand-navy text-white' : 'bg-white border border-brand-border text-brand-slate hover:border-brand-navy'}`}>All</button>
        {categories.map(c => (
          <button key={c} onClick={() => setCat(c)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${cat === c ? 'bg-brand-navy text-white' : 'bg-white border border-brand-border text-brand-slate hover:border-brand-navy'}`}>
      ...
      {loading ? (
        <p className="text-sm text-brand-slate">Loading vendors…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-brand-border p-10 text-center">
          ...
          <p className="text-sm text-brand-slate">No vendors in this category yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(v => (
            <div key={v.id} className="bg-white rounded-xl border border-brand-border p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-brand-ink">{v.name}</p>
                ...
                <button onClick={() => removeVendor(v.id)} className="p-1.5 text-brand-slate-light hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map(n => (
                  <Star key={n} className={`w-3.5 h-3.5 ${n <= (v.rating || 0) ? 'text-amber-400 fill-amber-400' : 'text-brand-border fill-brand-border'}`} />
                ))}
              </div>
              ...
              {v.performance_notes && (
                <p className="text-[11px] text-brand-slate-light mt-3 p-2.5 bg-brand-bg rounded-xl leading-relaxed">{v.performance_notes}</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-brand-ink">Add Vendor</h3>
            ...
            <form onSubmit={addVendor} className="space-y-3">
              ...
              <textarea placeholder="Performance notes (e.g. 24h turnaround, bulk discounts)" value={form.performance_notes} onChange={e => setForm({ ...form, performance_notes: e.target.value })} rows={3} className="w-full px-4 py-3 border border-brand-border rounded-3xl text-sm outline-none focus:border-brand-navy resize-none" />
              <button type="submit" className="w-full py-2.5 bg-brand-navy text-white text-sm font-semibold rounded-full hover:bg-brand-blue">Add Vendor</button>
            </form>
          </div>
        </div>
      )}
