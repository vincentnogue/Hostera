  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-brand-ink">Revenue Management</h1>
          <p className="text-sm text-brand-slate mt-1">Seasonal pricing, restrictions and automated rate adjustments</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-brand-border overflow-hidden">
        {ratePlans.length === 0 ? (
          <div className="py-16 text-center">
            <TrendingUp className="w-12 h-12 text-brand-border mx-auto mb-3" />
            <p className="text-sm text-brand-slate">No rate plans defined yet</p>
          </div>
        ) : (
          <div className="divide-y divide-brand-border">
            {ratePlans.map((plan) => {
              ...
              return (
                <div key={plan.id} className="p-4 flex items-start justify-between gap-4 hover:bg-brand-bg transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h3 className="text-sm font-semibold text-brand-ink">{plan.name}</h3>
                    ...
                    <p className="text-xs text-brand-slate mb-1">{roomType?.name || 'All room types'}</p>
                    <div className="flex items-center gap-3 text-xs text-brand-slate flex-wrap">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-lg font-bold text-brand-navy">${plan.price} <span className="text-xs text-brand-slate font-normal">{plan.currency}</span></p>
                    <button
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-brand-ink">New Rate Plan</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">Plan Name</label>
                ...
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">Room Type</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">Price / Night</label>
                  <input type="number" min="0" value={form.price} onChange={e => setForm({...form, price: parseFloat(e.target.value) || 0})} className={inputCls} />
                </div>
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">Min Stay</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">Start Date</label>
                  <input type="date" value={form.start_date} onChange={e => setForm({...form, start_date: e.target.value})} className={inputCls} />
                </div>
                <div>
                  <label className="text-sm font-medium text-brand-ink mb-1 block">End Date</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <label key={c.key} className="flex items-center gap-2 text-sm text-brand-ink">
                    <input type="checkbox" checked={form[c.key]} onChange={e => setForm({...form, [c.key]: e.target.checked})} className="accent-brand-navy" />
                    {c.label}
                  </label>
                ))}
              </div>
              <div className="p-3 bg-brand-bg rounded-lg space-y-2">
                <label className="flex items-center gap-2 text-sm text-brand-ink">
                  <input type="checkbox" checked={form.auto_adjust} onChange={e => setForm({...form, auto_adjust: e.target.checked})} className="accent-brand-navy" />
                  Enable automated rate adjustment
                </label>
                {form.auto_adjust && (
                  <div>
                    <label className="text-xs text-brand-slate mb-1 block">Adjustment % (can be negative for discounts)</label>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreate} disabled={creating || !form.name || !form.room_type_id} className="flex-1 px-4 py-2 bg-brand-navy text-white rounded-lg text-sm font-medium hover:bg-brand-blue disabled:opacity-50">
