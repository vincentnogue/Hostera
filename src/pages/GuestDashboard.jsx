    ...
    <div className="min-h-screen bg-[#F6F8FB]">
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-[#E2E8F0] px-4 lg:px-8 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-base font-bold tracking-tight text-[#17212B]">HOSTERA</p>
            <p className="text-[9px] text-[#94A3B8] uppercase tracking-widest">Guest</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <a href="#my-stays" className="hidden sm:inline-block px-4 py-2 text-sm font-medium text-[#123B63] hover:underline">My stays</a>
          <span className="w-9 h-9 rounded-full bg-[#123B63] text-white flex items-center justify-center text-sm font-medium">
            {(me?.email || "G").charAt(0).toUpperCase()}
          </span>
          <button onClick={() => db.auth.logout()} className="p-2 text-[#64748B] hover:text-[#123B63]" title="Log out">
      ...
      <section className="relative h-[420px] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#0E243F]/90 via-[#0E243F]/40 to-[#0E243F]/20" />
          ...
          <div className="mt-8 bg-white rounded-full shadow-xl p-2 flex flex-col md:flex-row items-center gap-2">
            <div className="flex-1 flex items-center gap-2 px-4 w-full">
              ...
              <input
                ...
                className="w-full py-2 text-sm outline-none text-[#17212B] placeholder:text-[#94A3B8]"
              />
            </div>
            <a href="#explore" className="shrink-0 px-6 py-2.5 rounded-full bg-[#123B63] text-white text-sm font-semibold hover:bg-[#1F5A8A]">
              Explore stays
            </a>
          </div>
        </div>
      ...
      <section id="explore" className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-xl font-bold text-[#17212B] mb-5 flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-[#1F5A8A]" /> Available properties
        </h2>
        {loading ? (
          <p className="text-sm text-[#64748B]">Loading properties…</p>
        ) : filteredProperties.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-[#E2E8F0] p-10 text-center text-sm text-[#64748B]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredProperties.map((p, i) => {
              ...
              return (
                <div key={p.id} className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden hover:shadow-xl transition-shadow">
                  <Image src={photo} alt={p.name} className="w-full h-44 block rounded-t-2xl" />
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="text-base font-semibold text-[#17212B]">{p.name}</h3>
                      {p.star_rating ? (
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <p className="text-sm font-semibold text-[#123B63]">
                        {minPrice != null ? <>From {p.currency || "USD"} {minPrice}<span className="text-xs font-normal text-[#64748B]"> /night</span></> : "Rates on request"}
                      </p>
                      <button onClick={() => openBooking(p)} className="px-5 py-2 rounded-full bg-[#123B63] text-white text-xs font-semibold hover:bg-[#1F5A8A]">
                        Book now
                      </button>
                    </div>
                  </div>
      ...
      <section id="my-stays" className="max-w-7xl mx-auto px-6 pb-16">
        <h2 className="text-xl font-bold text-[#17212B] mb-5 flex items-center gap-2">
          <CalendarDays className="w-5 h-5 text-[#1F5A8A]" /> My stays
        </h2>
        {stays.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-[#E2E8F0] p-10 text-center">
            <p className="text-sm text-[#64748B] mb-3">You have no bookings yet.</p>
            <a href="#explore" className="inline-block px-5 py-2 rounded-full bg-[#123B63] text-white text-xs font-semibold">Find a stay</a>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {stays.map(r => {
              ...
              return (
                <div key={r.id} className="bg-white rounded-2xl border border-[#E2E8F0] p-5 flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold text-[#17212B]">{prop?.name || "Your booking"}</p>
                    <p className="text-xs text-[#64748B] mt-0.5">{type?.name || "Room"} · {r.adults} adult{r.adults > 1 ? "s" : ""}{r.children ? ` · ${r.children} child${r.children > 1 ? "ren" : ""}` : ""}</p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-sm font-bold text-[#123B63] mt-2">{r.currency} {r.total_amount}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setBookingProperty(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-bold text-[#17212B]">Book {bookingProperty.name}</h3>
                <p className="text-xs text-[#64748B]">{[bookingProperty.city, bookingProperty.country].filter(Boolean).join(", ")}</p>
              </div>
              <button onClick={() => setBookingProperty(null)} className="p-1.5 text-[#64748B] hover:text-[#17212B]"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-[#64748B] block mb-1">Room type</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-[#64748B] block mb-1">Check-in</label>
                  <input type="date" min={today()} value={form.check_in} onChange={(e) => setForm({ ...form, check_in: e.target.value })} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs font-medium text-[#64748B] block mb-1">Check-out</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-[#64748B] block mb-1">Adults</label>
                  <input type="number" min={1} max={6} value={form.adults} onChange={(e) => setForm({ ...form, adults: e.target.value })} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs font-medium text-[#64748B] block mb-1">Children</label>
                  <input type="number" min={0} max={6} value={form.children} onChange={(e) => setForm({ ...form, children: e.target.value })} className={inputCls} />
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-[#64748B] block mb-1">Special requests (optional)</label>
              ...
              {selectedType && (
                <div className="bg-[#F6F8FB] rounded-xl px-4 py-3 flex items-center justify-between text-sm">
                  <span className="text-[#64748B]">{nights()} night{nights() > 1 ? "s" : ""} × {selectedType.currency || "USD"} {selectedType.base_price}</span>
                  <span className="font-bold text-[#123B63]">{selectedType.currency || "USD"} {estimatedTotal}</span>
                </div>
              )}
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A] disabled:opacity-50"
                >
                  <CheckCircle2 className="w-4 h-4" /> {saving ? "Booking…" : "Confirm booking"}
                </button>
                <button onClick={() => setBookingProperty(null)} className="flex-1 py-2.5 border border-[#E2E8F0] text-sm font-medium rounded-full text-[#64748B]">Cancel</button>
              </div>
            </div>
          </div>
        </div>
