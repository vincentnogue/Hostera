...
const typeConfig = { email: { icon: Mail, pill: 'bg-blue-50 text-brand-navy' }, promo: { icon: Tag, pill: 'bg-amber-50 text-amber-700' }, sms: { icon: MessageSquare, pill: 'bg-green-50 text-green-700' } };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand-ink">Marketing Tools</h1>
          <p className="text-sm text-brand-slate">Email campaigns, promotional offers and guest communication templates.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {Object.keys(typeConfig).map(t => (
          <button key={t} onClick={() => setType(t)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${type === t ? 'bg-brand-navy text-white' : 'bg-white border border-brand-border text-brand-slate hover:border-brand-navy'}`}>{t}</button>
        ))}
      </div>

      {loading ? (
        <p className="text-sm text-brand-slate">Loading campaigns…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-brand-border p-10 text-center">
          ...
          <p className="text-sm text-brand-slate">No campaigns yet — build your first one.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} className="bg-white rounded-xl border border-brand-border p-5 flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`text-[10px] px-2.5 py-1 rounded-full font-semibold capitalize ${c.status === 'sent' ? 'bg-green-50 text-green-700' : c.status === 'scheduled' ? 'bg-blue-50 text-brand-navy' : 'bg-gray-100 text-gray-500'}`}>
                    {c.status}
                  </span>
                </div>
                {c.subject && <p className="text-[12px] font-medium text-brand-ink mb-1">&quot;{c.subject}&quot;</p>}
                {c.content && <p className="text-[12px] text-brand-slate leading-relaxed mb-3 line-clamp-2 flex-1">{c.content}</p>}
                <div className="flex items-center justify-between pt-3 border-t border-[#F1F5F9]">
                  <span className="text-[10px] text-brand-slate-light">{audienceLabels[c.audience] || c.audience} · {c.recipients || audienceCount(c.audience)} recipients</span>
                  {c.status === 'draft' ? (
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-brand-ink">New Campaign</h3>
            ...
            <form onSubmit={addCampaign} className="space-y-3">
              ...
              <textarea placeholder="Message content…" value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={5} className="w-full px-4 py-3 border border-brand-border rounded-3xl text-sm outline-none focus:border-brand-navy resize-none" />
              <button type="submit" className="w-full py-2.5 bg-brand-navy text-white text-sm font-semibold rounded-full hover:bg-brand-blue">Create Campaign</button>
            </form>
          </div>
        </div>
      )}
