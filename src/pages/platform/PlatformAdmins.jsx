...
export default function PlatformAdmins() {
  ...
  const inputCls = "w-full px-3.5 py-2 border border-white/10 rounded-full text-sm bg-white/5 text-white outline-none focus:border-brand-blue placeholder:text-white/30";
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between">
        ...
        {isOwner && (
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-4 py-2.5 bg-brand-blue text-white text-sm font-semibold rounded-full hover:bg-[#2A6E9E]">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {PLATFORM_OWNERS.map(email => (
            <div key={email} className="p-5 rounded-xl bg-brand-navy border border-brand-blue/50 flex items-center gap-4">
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-brand-navy-950 rounded-2xl p-6 w-full max-w-md border border-white/10" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={addAdmin} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-brand-blue text-white text-sm font-semibold rounded-full hover:bg-[#2A6E9E]">Grant Access</button>
            </form>
          </div>
        </div>
      )}
