  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {flags.length === 0 ? (
          <div className="col-span-full bg-brand-navy-900 border border-white/5 rounded-xl py-16 text-center">
            <Flag className="w-12 h-12 text-white/10 mx-auto mb-3" />
            <p className="text-sm text-white/40">No feature flags defined</p>
          </div>
        ) : (
          flags.map(flag => (
            <div key={flag.id} className="bg-brand-navy-900 border border-white/5 rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-sm font-semibold text-white">{flag.name}</h3>
                  <code className="text-[10px] text-brand-blue font-mono">{flag.key}</code>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full text-xs px-2 py-1.5 rounded-lg bg-brand-navy border border-brand-blue/40 text-white outline-none cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs px-2 py-1.5 rounded-lg bg-brand-navy border border-brand-blue/40 text-white outline-none"
                  />
                </div>
              </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-brand-navy-900 border border-white/10 rounded-xl shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreate} disabled={creating || !form.key || !form.name} className="flex-1 px-4 py-2 bg-brand-blue text-white rounded-lg text-sm font-medium hover:bg-[#2563EB] disabled:opacity-50">
