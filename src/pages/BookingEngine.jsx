...
export default function BookingEngine() {
  ...
  const numInput = (key, label, suffix) => (
    <div>
      <label className="text-xs font-medium text-[#64748B] block mb-1.5">{label}</label>
      <div className="relative">
        <input
          type="number"
          value={config[key] ?? 0}
          onChange={e => setConfig({ ...config, [key]: Number(e.target.value) })}
          className="w-full px-4 py-2.5 border border-[#E2E8F0] rounded-full text-sm outline-none focus:border-[#123B63] pr-12"
        />
        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-[#94A3B8]">{suffix}</span>
      </div>
    </div>
  );

  if (loading) {
    return <p className="text-sm text-[#64748B]">Loading booking engine settings…</p>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Booking Engine</h1>
          <p className="text-sm text-[#64748B]">Direct reservation settings, availability display and booking rules.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
          <h3 className="text-sm font-semibold text-[#17212B] mb-5 flex items-center gap-2">
            <Globe className="w-4 h-4 text-[#123B63]" /> Booking Rules
          </h3>
          <div className="space-y-4">
            {toggles.map(t => (
              <div key={t.key} className="flex items-start justify-between gap-4 p-3.5 rounded-xl bg-[#F6F8FB]">
                <div>
                  <p className="text-sm font-medium text-[#17212B]">{t.label}</p>
                  <p className="text-[11px] text-[#64748B] mt-0.5 leading-relaxed">{t.desc}</p>
                </div>
                <button
                  onClick={() => setConfig({ ...config, [t.key]: !config[t.key] })}
                  className={`shrink-0 w-11 h-6 rounded-full relative transition-colors ${config[t.key] ? 'bg-[#123B63]' : 'bg-[#E2E8F0]'}`}
                >
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-6 space-y-5">
          <h3 className="text-sm font-semibold text-[#17212B] flex items-center gap-2">
            <Check className="w-4 h-4 text-[#123B63]" /> Stay & Payment Rules
          </h3>
          ...
          <div>
            <label className="text-xs font-medium text-[#64748B] block mb-1.5">Cancellation policy (shown to guests)</label>
            <textarea
              ...
              className="w-full px-4 py-3 border border-[#E2E8F0] rounded-3xl text-sm outline-none focus:border-[#123B63] resize-none"
            />
          </div>
          <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl">
            <p className="text-xs text-[#123B63] leading-relaxed">
