  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Inventory Management</h1>
          <p className="text-sm text-[#64748B]">Housekeeping supplies, linen stock and operational assets.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Tracked Items', value: items.length, icon: Boxes, tint: 'text-[#123B63]' },
          ...
          { label: 'Suppliers', value: new Set(items.map(i => i.supplier).filter(Boolean)).size, icon: Truck, tint: 'text-[#1F5A8A]' },
        ].map(k => {
          const Icon = k.icon;
          return (
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setCat('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${cat === 'all' ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>All</button>
        {categories.map(c => (
          <button key={c} onClick={() => setCat(c)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${cat === c ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>{c}</button>
        ))}
      </div>

      {loading ? (
        <p className="text-sm text-[#64748B]">Loading inventory…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-[#E2E8F0] p-10 text-center">
          ...
          <p className="text-sm text-[#64748B]">No items in this category.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={i.id} className="border-b border-[#F1F5F9] last:border-0">
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-[#17212B]">{i.name}</p>
                      <p className="text-[11px] text-[#94A3B8]">Restocked {i.last_restocked ? new Date(i.last_restocked).toLocaleDateString() : '—'}</p>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="text-[11px] px-2.5 py-1 rounded-full bg-[#F6F8FB] text-[#64748B] capitalize">{i.category}</span>
                    </td>
                    <td className="px-5 py-3.5 w-44">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-semibold ${low ? 'text-red-600' : 'text-[#17212B]'}`}>{i.quantity}</span>
                        <span className="text-[10px] text-[#94A3B8]">/ {i.min_stock} {i.unit}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-[#17212B]">${(i.unit_cost || 0).toFixed(2)}</td>
                    <td className="px-5 py-3.5 text-[#64748B]">{i.supplier || '—'}</td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="inline-flex gap-1.5">
                        <button onClick={() => { setRestocking(i.id); setRestockQty(Math.max(10, i.min_stock || 10)); }} className="px-3 py-1.5 bg-[#123B63] text-white text-xs font-semibold rounded-full hover:bg-[#1F5A8A]">Restock</button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#17212B]">Add Inventory Item</h3>
            ...
            <form onSubmit={addItem} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Add Item</button>
            </form>
          </div>
        </div>
      )}
      ...
      {restocking && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setRestocking(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-[#17212B] mb-2">Restock {items.find(i => i.id === restocking)?.name}</h3>
            <p className="text-xs text-[#64748B] mb-4">Current: {items.find(i => i.id === restocking)?.quantity} units</p>
            <input type="number" value={restockQty} onChange={e => setRestockQty(e.target.value)} className={inputCls} />
            <div className="flex gap-2 mt-4">
              <button onClick={confirmRestock} className="flex-1 py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Confirm</button>
              <button onClick={() => setRestocking(null)} className="flex-1 py-2.5 border border-[#E2E8F0] text-sm font-medium rounded-full text-[#64748B]">Cancel</button>
            </div>
          </div>
        </div>
      )}
