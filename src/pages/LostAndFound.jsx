...
const statusPills = {
  found: 'bg-blue-50 text-[#123B63]', stored: 'bg-amber-50 text-amber-700', claimed: 'bg-purple-50 text-purple-700',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Lost & Found</h1>
          <p className="text-sm text-[#64748B]">Record, track and manage guest items found on property — from discovery to return.</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 md:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          {['all', 'found', 'stored', 'claimed', 'returned', 'donated', 'disposed'].map(s => (
            <button key={s} onClick={() => setStatus(s)} className={`px-3.5 py-1.5 text-xs font-medium rounded-full capitalize ${status === s ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>{s}</button>
          ))}
        </div>
      </div>

      {loading ? (
        <p className="text-sm text-[#64748B]">Loading items…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-[#E2E8F0] p-10 text-center">
          ...
          <p className="text-sm text-[#64748B]">No items match your filters.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(i => (
            <div key={i.id} className="bg-white rounded-xl border border-[#E2E8F0] p-5 flex flex-col">
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-full bg-[#F6F8FB] flex items-center justify-center">
                  <PackageSearch className="w-4 h-4 text-[#123B63]" />
                </div>
              ...
              <p className="text-sm font-semibold text-[#17212B]">{i.item_name}</p>
              {i.description && <p className="text-[12px] text-[#64748B] mt-1 leading-relaxed">{i.description}</p>}
              <div className="space-y-1 mt-3 text-[11px] text-[#64748B]">
                ...
                {i.found_by && <p className="text-[#94A3B8]">Found by {i.found_by}</p>}
              </div>
              <div className="flex items-center gap-2 pt-3 mt-auto border-t border-[#F1F5F9] mt-3">
                {STATUS_FLOW[i.status] && (
                  <button onClick={() => advance(i)} className="flex items-center gap-1 px-3 py-1.5 bg-[#123B63] text-white text-[11px] font-semibold rounded-full hover:bg-[#1F5A8A] capitalize">
                    {STATUS_FLOW[i.status]} <ArrowRight className="w-3 h-3" />
                  </button>
                )}
                {i.status === 'stored' && (
                  <>
                    <button onClick={() => markFinal(i, 'donated')} className="px-3 py-1.5 border border-[#E2E8F0] text-[11px] text-[#64748B] rounded-full hover:border-[#123B63]">Donate</button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#17212B]">Record Found Item</h3>
            ...
            <form onSubmit={addItem} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Record Item</button>
            </form>
          </div>
        </div>
      )}
