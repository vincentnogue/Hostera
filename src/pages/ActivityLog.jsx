...
export default function ActivityLog() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-brand-ink">Activity Log</h1>
        <p className="text-sm text-brand-slate">Chronological record of all system actions, changes and user activities.</p>
      </div>
      ...
      {loading ? (
        <p className="text-sm text-brand-slate">Loading activity…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-brand-border p-10 text-center">
          ...
          <p className="text-sm text-brand-slate">No activity recorded yet.</p>
        </div>
      ) : (
                ...
                <div key={log.id} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full bg-brand-bg flex items-center justify-center shrink-0`}>
                      <Icon className={`w-4 h-4 ${log.severity === 'critical' ? 'text-red-500' : log.severity === 'warning' ? 'text-amber-500' : 'text-brand-blue'}`} />
                    </div>
                    {i < filtered.length - 1 && <div className="w-px flex-1 bg-brand-border my-1" />}
                  </div>
                  <div className="flex-1 pb-6">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {log.entity_type && <span className="text-[10px] px-2 py-0.5 rounded-full bg-brand-bg text-brand-slate border border-brand-border">{log.entity_type}</span>}
                    </div>
                    {log.details && <p className="text-[13px] text-brand-slate mt-1 leading-relaxed">{log.details}</p>}
                    <div className="flex items-center gap-4 mt-1.5 text-[11px] text-brand-slate-light">
