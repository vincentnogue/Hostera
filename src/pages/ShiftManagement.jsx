  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17212B]">Shift Management</h1>
          <p className="text-sm text-[#64748B]">Working hours, shift handovers and team availability across departments.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setDept('all')} className={`px-3.5 py-1.5 text-xs font-medium rounded-full ${dept === 'all' ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:border-[#123B63]'}`}>All Departments</button>
        {departments.map(d => (
      ...
      {loading ? (
        <p className="text-sm text-[#64748B]">Loading shifts…</p>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-[#E2E8F0] p-10 text-center">
          ...
          <p className="text-sm text-[#64748B]">No shifts scheduled for this department.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map(s => {
            ...
            return (
              <div key={s.id} className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex flex-col md:flex-row md:items-center gap-3">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold text-[#17212B] truncate">{s.staff_name}</p>
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#F6F8FB] text-[#64748B] border border-[#E2E8F0] capitalize">{deptLabels[s.department] || s.department}</span>
                    </div>
                    <p className="text-[11px] text-[#64748B] mt-0.5">{s.date === today ? 'Today' : new Date(s.date).toLocaleDateString()} · {s.start_time} – {s.end_time}</p>
                  </div>
                </div>
                {s.handover_notes && (
                  <div className="md:max-w-xs w-full bg-[#F6F8FB] rounded-xl p-2.5 border-l-2 border-[#123B63]">
                    <p className="text-[10px] font-semibold text-[#123B63] uppercase tracking-wide">Handover</p>
                    <p className="text-[12px] text-[#17212B] leading-snug line-clamp-2">{s.handover_notes}</p>
                  </div>
                )}
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-[10px] px-2.5 py-1 rounded-full font-semibold capitalize ${s.status === 'completed' ? 'bg-green-50 text-green-700' : s.status === 'in_progress' ? 'bg-blue-50 text-[#123B63]' : s.status === 'cancelled' ? 'bg-gray-100 text-gray-500' : 'bg-amber-50 text-amber-700'}`}>
                    {s.status.replace('_', ' ')}
                  </span>
                  {nextStatus[s.status] && (
                    <button onClick={() => advance(s)} className="flex items-center gap-1 px-3 py-1.5 bg-[#123B63] text-white text-[11px] font-semibold rounded-full hover:bg-[#1F5A8A] capitalize">
                      {nextStatus[s.status]} <ArrowRight className="w-3 h-3" />
                    </button>
                  )}
                  {s.status === 'completed' && (
                    <button onClick={() => { setHandoverEdit(s.id); setHandoverText(s.handover_notes || ''); }} className="flex items-center gap-1 px-3 py-1.5 border border-[#E2E8F0] text-[#64748B] text-[11px] font-medium rounded-full hover:border-[#123B63] hover:text-[#123B63]">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-[#17212B]">Schedule Shift</h3>
              <button onClick={() => setShowAdd(false)}><X className="w-4 h-4 text-[#64748B]" /></button>
            </div>
            <form onSubmit={addShift} className="space-y-3">
              <div>
                <label className="text-xs font-medium text-[#64748B] block mb-1.5">Team member</label>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {staff.slice(0, 8).map(m => (
                    <button type="button" key={m.id} onClick={() => setForm({ ...form, staff_name: m.full_name, department: m.department })} className={`px-3 py-1.5 text-xs rounded-full border ${form.staff_name === m.full_name ? 'bg-[#123B63] text-white border-[#123B63]' : 'border-[#E2E8F0] text-[#64748B]'}`}>
                      {m.full_name}
                    </button>
                  ))}
                </div>
              ...
              <button type="submit" className="w-full py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Schedule</button>
            </form>
          </div>
        </div>
      )}
      ...
      {handoverEdit && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setHandoverEdit(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-[#17212B] mb-1">Shift Handover Notes</h3>
            <p className="text-xs text-[#64748B] mb-4">What the next team needs to know — VIP arrivals, pending issues, follow-ups.</p>
            <textarea value={handoverText} onChange={e => setHandoverText(e.target.value)} rows={5} placeholder="e.g. Suite 502 AC still noisy; VIP arrival at 9pm — Mrs. Laurent; 3 pending wake-up calls…" className="w-full px-4 py-3 border border-[#E2E8F0] rounded-3xl text-sm outline-none focus:border-[#123B63] resize-none" />
            <div className="flex gap-2 mt-4">
              <button onClick={saveHandover} className="flex-1 py-2.5 bg-[#123B63] text-white text-sm font-semibold rounded-full hover:bg-[#1F5A8A]">Save Handover</button>
              <button onClick={() => setHandoverEdit(null)} className="flex-1 py-2.5 border border-[#E2E8F0] text-sm font-medium rounded-full text-[#64748B]">Cancel</button>
            </div>
          </div>
        </div>
      )}
