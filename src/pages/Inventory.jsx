  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B] mt-1">Track supplies, minibar stock and maintenance parts</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Items', value: totalItems, icon: Package, color: 'text-[#123B63]', bg: 'bg-blue-50' },
      ...
      {lowStockItems.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-sm font-semibold text-[#17212B]">Low Stock Alert ({lowStockItems.length} items)</h3>
          </div>
          <div className="flex flex-wrap gap-2">
            {lowStockItems.map(i => (
              <span key={i.id} className="text-xs bg-white px-2.5 py-1 rounded-lg border border-orange-200 text-[#17212B]">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === key ? 'bg-[#123B63] text-white' : 'bg-white border border-[#E2E8F0] text-[#64748B] hover:bg-[#F6F8FB]'
              }`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white rounded-xl border border-[#E2E8F0] p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-sm font-semibold text-[#17212B]">{item.name}</h3>
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-sm font-bold ${isLow ? 'text-orange-600' : 'text-[#17212B]'}`}>
                  ...
                  <p className="text-[10px] text-[#64748B] mt-1">Minimum: {item.min_stock || 0}</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 border border-[#E2E8F0] rounded-lg text-xs font-medium text-[#64748B] hover:bg-[#F6F8FB] transition-colors disabled:opacity-40"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-[#123B63] text-white rounded-lg text-xs font-medium hover:bg-[#1F5A8A] transition-colors disabled:opacity-50"
                  >
