    ...
    <div className="bg-white">
      ...
      <section className="relative bg-brand-navy py-20 overflow-hidden">
        <div className="absolute inset-0">
          <img src="/images/hotels/hotel-10-iconic-tower.jpg" className="w-full h-full object-cover opacity-80" alt="Iconic tower — Dubai" />
        ...
        <div className="max-w-7xl mx-auto px-6">
          <Reveal>
            <div className="text-center mb-14">
              <h2 className="text-3xl font-bold text-brand-ink mb-3">Find your industry</h2>
              <p className="text-brand-slate max-w-2xl mx-auto">Each industry gets the same powerful core — with the workflows that matter most to you highlighted.</p>
            </div>
          </Reveal>
            ...
            {INDUSTRIES.map((ind, i) => {
              ...
              return (
                <Reveal key={ind.name} delay={(i % 4) * 0.06}>
                  <div className="p-6 rounded-2xl border border-brand-border hover:border-brand-blue/40 hover:shadow-xl transition-all h-full flex flex-col">
                    <div className="w-11 h-11 rounded-full bg-brand-bg flex items-center justify-center mb-4">
                      <Icon className="w-5 h-5 text-brand-navy" />
                    </div>
                    <h3 className="text-base font-semibold text-brand-ink mb-1.5">{ind.name}</h3>
                    <p className="text-[13px] text-brand-slate leading-relaxed mb-4">{ind.blurb}</p>
                    <ul className="space-y-1.5 mt-auto">
                      {ind.points.map(p => (
                        <li key={p} className="flex items-center gap-2 text-xs text-brand-ink">
      ...
      <section className="py-20 bg-brand-bg">
        <div className="max-w-7xl mx-auto px-6">
          <Reveal>
            <div className="text-center mb-14">
              <h2 className="text-3xl font-bold text-brand-ink mb-3">The same platform, shaped to your operation</h2>
              <p className="text-brand-slate max-w-2xl mx-auto">Property type, size, currency, timezone and language — all configurable per property.</p>
            </div>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <Reveal key={c.title} delay={i * 0.1}>
                <div className="p-8 rounded-2xl bg-white border border-brand-border h-full">
                  <span className="inline-block px-3 py-1 bg-blue-50 text-brand-navy text-xs font-semibold rounded-full mb-4">{c.point}</span>
                  <h3 className="text-lg font-bold text-brand-ink mb-2.5">{c.title}</h3>
                  <p className="text-sm text-brand-slate leading-relaxed">{c.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
